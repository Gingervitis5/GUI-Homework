...
public class UndoableScribble extends AbstractUndoableEdit{
    ...
    public UndoableScribble(Canvas newCanvas){
    ...
    @Override
    public void undo(){
        WritableImage temp = mSceneCanvas.snapshot(null, null);
        WritableImage undo = mBackup.snapshot(null, null);
        GraphicsContext gc = mSceneCanvas.getGraphicsContext2D();
        gc.drawImage(undo, 0, 0);
        gc = mBackup.getGraphicsContext2D();
        gc.drawImage(temp, 0, 0);
    }
    
    @Override
    public void redo(){
        WritableImage temp = mBackup.snapshot(null, null);
        WritableImage undo = mSceneCanvas.snapshot(null, null);
        GraphicsContext gc = mSceneCanvas.getGraphicsContext2D();
        gc.drawImage(temp, 0, 0);
        gc = mBackup.getGraphicsContext2D();
        gc.drawImage(undo, 0, 0);
